...
class SimpleKeyTeleop():
    def __init__(self):
        ...
        self._forward_rate = 0.786
        self._backward_rate = 0.5
        self._rotation_rate = 1.014
        self._angular = 0
        self._linear = 0
    ...
    def run(self):
        self.set_state(4, 8, math.pi)
        time.sleep(0.5)

        self.read_pose();
        for i in range(8):
            self.step(curses.KEY_LEFT, 15)
            time.sleep(1.8)

            self.step(curses.KEY_UP, 25)
            time.sleep(2.5)

            self.step(curses.KEY_RIGHT, 15)
            time.sleep(1.8)
